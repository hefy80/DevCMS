...
class StoryModel extends Model{
	 ...
	 * 获取指定外部bug列表
	 */
	public function getBugOutsideStat(){
		$res = S('devcms:BugOutsideStat');
		if ($res && count($res)>0)
			return $res;
	...
	//	dump($res['2016']['stat']['11']['stat']); exit();
		S('devcms:BugOutsideStat',$res,60);
		return (!$res) ? false : $res;
	}
	 ...
	 * 获取二线统计列表
	 */
	public function getLine2Stat(){
		$res = S('devcms:Line2Stat');
		if ($res && count($res)>0)
			return $res;
...
//		dump($res['2016']['stat']['11']['list']); exit();
	 ...
	 * 获取三线统计列表
	 */
	public function getLine3Stat(){
		$res = S('devcms:Line3Stat');
		if ($res && count($res)>0)
			return $res;
...
//					echo substr($val['title'],0,9);
	...
	function getProducts(){
		...
		$bugs = $this->query("
			select 
				pd.id as '编号', pd.name as '产品名', 
				sum(case when bg.deleted = '0' and bg.status != 'closed' then 1 else 0 end) as '未解决bug数'
			from 
		...
		foreach ($storys as $key => $value) {
			...
			{
				$res[$value['id']]['待做需求数']+=1;
			}
			else if ($value['stage']!='release')
			{
				$res[$value['id']]['开发中的需求'][]=array('id'=>$value['sid'],'title'=>$value['title']);
			}
		}

		S('devcms:products',$res,600);
		return (!$res) ? false : $res;
	}
